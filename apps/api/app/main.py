...
def admin(x_admin_key):
    if not db.auth_admin(x_admin_key): raise HTTPException(401,'Admin authentication required')

def player(x_player_token):
    p=db.auth_player(x_player_token)
    if not p: raise HTTPException(401,'Invalid player token')
    return p

@app.get('/health')
def health(): return {'status':'ok','database':'configured' if os.getenv('DATABASE_URL') else 'local-sqlite'}
...
def register(tid:str,data:PlayerCreate):
    ...
    if error: raise HTTPException(409 if error not in ('TOURNAMENT_NOT_FOUND',) else 404,error)
...
def dispute(match_id:str,data:DisputeCreate,x_player_token:str|None=Header(default=None)):
    p=player(x_player_token); m=next((m for m in db.get_matches('') if m['id']==match_id),None)
